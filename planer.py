...
def main():
    ...
    print('(4) Edit event')
    print('(5) Search for event by name') #w zdarzenia list "q"
    ...
    if program == 1:
        ...
        try:
            ...
            print(f'Getting the upcoming x events')
            events_result = service.events().list(calendarId='primary', timeMin=input_date + 'T00:00:00-00:00', timeMax=input_date + 'T23:59:59-00:00', maxResults=10, singleEvents=True, orderBy='startTime').execute()
    ...
    if program == 2:
        numb_of_events = int(input('Select number of showing events: '))
        try:
            service = build('calendar', 'v3', credentials=creds)
